...
class Map():
    ...
    def __init__(self, wymiar):
        ...
        self.wymiar = wymiar
        self.dimx = wymiar  # columns
        self.dimy = wymiar  # rows
        ...
        #self.show()
        print('b')
        self.create_new_map(self.dimx, self.dimy)
        

    def create_new_map(self, dimx, dimy):   #
        self.map = [[ 0 for el in range(dimx)] for el in range(dimy)]

        
        self._ustawpolaprzejściowe()
        self._ustawpolanieprzejściowe()
        self._ustawprzedmioty()
        pass
    ...
    def _ustawpolaprzejściowe(self): # '.'
        for col in range(self.dimx):
            for row in range(self.dimy):
                self.map[col][row] = Room(col, row, is_walk_through=True) 

    def _ustawpolanieprzejściowe(self):  # '#'
        for i in range(2*self.wymiar): # 2 x więcej pól zakrytych niż wymiar mapy
            col = random.randint(0,self.wymiar-1)
            row = random.randint(0,self.wymiar-1)
            self.map[col][row] = Room(col, row, is_walk_through=False)

    def _ustawprzedmioty(self):
        for col in range(self.dimx):
            for row in range(self.dimy):
                if self.map[col][row].jaki() == True: # jeżeli pokój przejściowy
                    r = random.randint(0,1)
                    if r == 1:
                        self.map[col][row].przedmiot = Wartościowy()
                    else:
                        self.map[col][row].przedmiot = Śmieć()
